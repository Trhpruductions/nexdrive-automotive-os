...
export async function requestAppointment(formData: FormData) {
  ...
  await db.appointment.create({ data: { customerId: user.customerId, vehicleId, scheduledStart: start, scheduledEnd: addMinutes(start, 60), serviceRequested, notes: notes ? `Customer request: ${notes}` : "Requested via customer portal", dropOff: formData.has("dropOff"), status: "SCHEDULED" } });
