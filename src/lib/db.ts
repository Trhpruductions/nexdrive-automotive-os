...
export async function nextNumber(kind: "wo" | "inv" | "po" | "job" | "ship", shopId?: string): Promise<number> {
  const id = shopId ?? (await currentShopId());
  const field = { wo: "woSeq", inv: "invSeq", po: "poSeq", job: "jobSeq", ship: "shipSeq" }[kind] as "woSeq" | "invSeq" | "poSeq" | "jobSeq" | "shipSeq";
  const row = await rawDb.shop.update({ where: { id }, data: { [field]: { increment: 1 } }, select: { woSeq: true, invSeq: true, poSeq: true, jobSeq: true, shipSeq: true } });
  return row[field];
}
