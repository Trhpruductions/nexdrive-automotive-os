...
export function buildNav(enabled: ModuleKey[], role: string, labels: Partial<Record<ModuleKey, string>> = {}): NavItem[] {
  ...
  for (const m of MODULES) {
    ...
    items.push({ key: m.key, label: labels[m.key] ?? m.label, href: m.href, group: m.group });
  }
