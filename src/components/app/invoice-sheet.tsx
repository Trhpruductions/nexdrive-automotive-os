...
export function InvoiceSheet({
  ...
  vehicle: { year: number; make: string; model: string; trim?: string | null; vin?: string | null; licensePlate?: string | null; color?: string | null };
  lines: Line[];
    ...
    <div className="print-sheet card p-6 sm:p-8 text-sm">
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-6">
        ...
        <div className="sm:text-right">
          ...
          <div className="text-xs text-muted">{kind === "INVOICE" ? `Work order ${woNumber(workOrderNumber)} · ` : ""}{fmtDate(date)}</div>
      ...
      <div className="grid sm:grid-cols-2 gap-6 mt-8">
        ...
        <div>
          ...
          {mileageIn ? <div className="text-muted">Mileage {num(mileageIn)}</div> : null}
        </div>
      </div>
