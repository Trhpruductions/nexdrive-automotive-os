...
import { vehicleName, customerName } from "@/lib/format";
  ...
  return (
    <div className="overflow-x-auto">
      <div className="min-w-[640px]">
        ...
        {rows.map((row) => {
          const items = appointments.filter((a) => (row.id === "__none" ? !a.bay : a.bay?.id === row.id));
          return (
                ...
                {items.map((a) => {
                  ...
                  return (
                    <Link
                      ...
                      className={`absolute top-1 bottom-1 rounded-md border px-2 py-0.5 text-white overflow-hidden text-[11px] leading-tight hover:brightness-110 ${STATUS_BG[a.status] ?? STATUS_BG.SCHEDULED}`}
                      style={{ left: `${l}%`, width: `${Math.max(3, r - l)}%` }}
                    >
                      <div className="font-semibold truncate">{vehicleName(a.vehicle)}</div>
                      {!compact ? <div className="truncate opacity-90">{a.customer.firstName} {a.customer.lastName}</div> : null}
                    </Link>
                  );
                })}
              </div>
